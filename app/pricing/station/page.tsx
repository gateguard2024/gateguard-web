...
export default function PricingStationCalculator() {
  ...
  // --- URL ENCODING & DECODING (SHARE LINK) ---
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (quoteStr) {
        try {
          ...
          if (decoded.buildings && Array.isArray(decoded.buildings)) setBuildings(decoded.buildings);
        } catch (e) {
      ...
      <div className="hidden print:block bg-white text-black min-h-screen p-8 font-sans w-full max-w-[8.5in] mx-auto">
        ...
        <div className="flex justify-between items-end border-b-2 border-zinc-800 pb-6 mb-8">
          <div>
            ...
            <h2 className="text-lg font-bold text-zinc-500 uppercase tracking-widest">Access Control Options</h2>
          </div>
          <div className="text-right">
             <p className="text-sm font-bold uppercase tracking-widest text-zinc-400">Prepared For</p>
             <p className="text-xl font-black text-zinc-900">Columbia Residential</p>
          </div>
        </div>
        ...
        <div className="flex gap-8 mb-10">
          <div className="flex-1 bg-zinc-50 rounded-xl p-6 border border-zinc-200">
            <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-1">Total Buildings</p>
            <p className="text-3xl font-black text-zinc-900">{buildings.length}</p>
          </div>
          <div className="flex-1 bg-zinc-50 rounded-xl p-6 border border-zinc-200">
        ...
        {/* Matrix Comparison */}
        <h3 className="text-lg font-black uppercase tracking-widest text-zinc-900 mb-4 border-b border-zinc-200 pb-2">Financial Comparison</h3>
        <table className="w-full text-left mb-10 border-collapse">
          <thead>
            <tr className="border-b-2 border-zinc-800 text-xs uppercase tracking-widest text-zinc-500 bg-zinc-50">
              <th className="p-4 font-bold">Strategy</th>
              <th className="p-4 font-bold">Upfront CapEx</th>
              <th className="p-4 font-bold">Monthly OpEx</th>
              <th className="p-4 font-bold">Maintenance Model</th>
            </tr>
          </thead>
          <tbody className="text-sm">
            <tr className="border-b border-zinc-200 bg-blue-50/50">
              <td className="p-4 font-bold text-blue-900">Opt 1: Your Gate Guard (Rec)</td>
              <td className="p-4 font-mono font-black">${opt1CapEx.toLocaleString()}</td>
              <td className="p-4 font-mono font-black">${opt1OpEx.toLocaleString()} <span className="text-[10px] text-zinc-500 ml-1 font-sans font-medium">(${opt1CPU}/unit)</span></td>
              <td className="p-4 text-emerald-600 font-bold text-[10px] uppercase tracking-widest">Included (Proactive)</td>
            </tr>
            <tr className="border-b border-zinc-200">
              <td className="p-4 font-bold text-zinc-900">Opt 2: Re-Structure</td>
              <td className="p-4 font-mono text-zinc-700">${opt2CapEx.toLocaleString()}</td>
              <td className="p-4 font-mono text-zinc-700">${opt2OpEx.toLocaleString()} <span className="text-[10px] text-zinc-400 ml-1 font-sans font-medium">(${opt2CPU}/unit)</span></td>
              <td className="p-4 text-red-500 font-bold text-[10px] uppercase tracking-widest">Break-Fix (Billed Extra)</td>
            </tr>
            <tr className="border-b border-zinc-200">
              <td className="p-4 font-bold text-zinc-900">Opt 3: Keep & Reduce</td>
              <td className="p-4 font-mono text-zinc-700">${opt3CapEx.toLocaleString()}</td>
              <td className="p-4 font-mono text-zinc-700">${opt3OpEx.toLocaleString()} <span className="text-[10px] text-zinc-400 ml-1 font-sans font-medium">(${opt3CPU}/unit)</span></td>
              <td className="p-4 text-red-500 font-bold text-[10px] uppercase tracking-widest">Break-Fix (Billed Extra)</td>
            </tr>
            <tr className="border-b border-zinc-200">
              <td className="p-4 font-bold text-zinc-900">Opt 4: Simplify & Remove</td>
              <td className="p-4 font-mono text-zinc-700">${opt4CapEx.toLocaleString()}</td>
              <td className="p-4 font-mono text-zinc-700">${opt4OpEx.toLocaleString()} <span className="text-[10px] text-zinc-400 ml-1 font-sans font-medium">(${opt4CPU}/unit)</span></td>
              <td className="p-4 text-red-500 font-bold text-[10px] uppercase tracking-widest">Break-Fix (Billed Extra)</td>
            </tr>
          </tbody>
        </table>
        ...
        <h3 className="text-sm font-black uppercase tracking-widest text-zinc-900 mb-4 border-b border-zinc-200 pb-2">Building Infrastructure Summary</h3>
        <table className="w-full text-left mb-10 border-collapse text-sm">
          <thead>
            <tr className="border-b-2 border-zinc-800 text-[10px] uppercase tracking-widest text-zinc-500">
              <th className="p-3 font-bold">Building #</th>
              <th className="p-3 font-bold">Units</th>
              <th className="p-3 font-bold">Veh. Gates (Total / Broken)</th>
            </tr>
          </thead>
          <tbody>
            {buildings.map((b, i) => (
              <tr key={i} className="border-b border-zinc-100">
                <td className="p-3 font-bold text-zinc-800">{b.buildingNumber || `Building ${i+1}`}</td>
                <td className="p-3 text-zinc-600">{b.units}</td>
                <td className="p-3 text-zinc-600">{b.vehicleGates} / <span className={b.vehicleGatesRepair > 0 ? 'text-red-500 font-bold' : ''}>{b.vehicleGatesRepair}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {/* Definitions Snippet */}
        <div className="bg-zinc-50 p-6 rounded-xl text-xs text-zinc-600 border border-zinc-200 print:break-inside-avoid">
           <p className="font-bold text-zinc-900 uppercase tracking-widest mb-2 text-[10px]">Your Gate Guard vs. Legacy Options</p>
           <p className="leading-relaxed"><strong>Your Gate Guard (Opt 1)</strong> is a proactive, fully-managed service that absorbs physical hardware maintenance, provides modern app-based entry, and ensures high system uptime for a predictable flat monthly fee. Options 2-4 rely on a <strong>Break-Fix</strong> maintenance model, meaning repairs are only performed reactively after a component fails, resulting in unpredictable CapEx spikes and extended system downtime.</p>
        </div>
      </div>


      {/* 2. WEB APP VIEW (HIDDEN ON PRINT EXPORT) */}
      <main className="relative text-zinc-100 font-sans selection:bg-cyan-500/30 overflow-y-auto flex flex-col scroll-smooth print:hidden">
        <div className="fixed inset-0 z-[-2] bg-[#0A0A0C]"></div>
        <div className="fixed inset-0 z-[-1] bg-[radial-gradient(#ffffff05_1px,transparent_1px)] [background-size:24px_24px]"></div>
        ...
        <div className="max-w-[1800px] mx-auto w-full flex flex-col lg:flex-row relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className={`md:col-span-2 bg-[#0D0D12] border-2 rounded-xl overflow-hidden shadow-[0_0_20px_rgba(6,182,212,0.1)] relative transition-all ${activeScopeTab === 'opt1' ? 'border-cyan-400 scale-[1.02] z-10' : 'border-cyan-900/40 opacity-90'}`}>
                  ...
                  <div className="p-6 flex justify-between items-center gap-4">
                    <div>
                      <p className="text-[10px] uppercase tracking-widest text-cyan-500/70 font-bold">Setup CapEx</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-cyan-500/70 font-medium mt-1">CPU: ${opt1CPU}/unit</p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border-b border-white/5 space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[8px] text-zinc-500 font-medium">CPU: ${opt2CPU}/unit</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-4 border-b border-white/5 space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[8px] text-zinc-500 font-medium">CPU: ${opt3CPU}/unit</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="p-4 border-b border-white/5 space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[8px] text-zinc-500 font-medium">CPU: ${opt4CPU}/unit</p>
                      </div>
                    </div>
                  </div>
                </div>
